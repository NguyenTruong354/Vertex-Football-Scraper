...
## 1.3 — Fix news_radar.py: league_id hardcode

**File:** `news_radar.py`

**Vấn đề:** Tất cả tin BBC/Sky đều bị gán `league_id = "EPL"` kể cả tin Champions League,
chuyển nhượng, hay các giải khác. Query theo league sẽ bị sai.
...
```python
            ...
            """, (
                ...
                item["published_at"],
                item["source"],
                item.get("league_id"),  # None = tin tổng hợp, không ép buộc EPL
            ))
```

**Tiếp theo, tìm hàm `fetch_news`, tìm đoạn append:**
```python
                if title and link:
                    all_news.append({
                        "title": title,
...
```python
                if title and link:
                    # Detect league từ title để tag đúng
                    detected_league = _detect_league_from_title(title)
                    all_news.append({
                        "title": title,
                        "link": link,
                        "summary": summary,
...
**Thêm hàm mới `_detect_league_from_title` vào cuối file (trước `if __name__ == "__main__"`):**
```python
def _detect_league_from_title(title: str) -> str | None:
    """
    Detect league_id from news title keywords.
    Returns league code string or None if cannot determine.
    """
    title_lower = title.lower()
    LEAGUE_KEYWORDS = {
        "EPL":          ["premier league", "epl", "man united", "man city", "liverpool",
                         "arsenal", "chelsea", "tottenham", "newcastle"],
        "LALIGA":       ["la liga", "laliga", "real madrid", "barcelona", "atletico"],
        "BUNDESLIGA":   ["bundesliga", "bayern", "dortmund", "bvb"],
        "SERIE_A":      ["serie a", "juventus", "inter milan", "ac milan", "napoli"],
        "LIGUE_1":      ["ligue 1", "psg", "paris saint-germain", "marseille"],
        "UCL":          ["champions league", "ucl", "europa league"],
    }
    for league_id, keywords in LEAGUE_KEYWORDS.items():
        if any(kw in title_lower for kw in keywords):
            return league_id
    return None  # Tin tổng hợp
```
...
```python
    ...
    last_insight = _get_last_published_insight(
        # event_id cần được truyền vào hàm analyze — xem bước 4.2
        getattr(analyze, "_current_event_id", None)
    )
    avoid_repeat_block = (
...
Và thêm ngay sau dòng `if minute < 15: return 0, ""`:
```python
    # Lưu event_id để _get_last_published_insight dùng được
    analyze._current_event_id = event_id
```
...
| Phần | Task | Mức độ | Lý do |
|------|------|--------|-------|
| 1.1 | Fix language bug live_insight | 🔴 BẮT BUỘC | Bug hiện tại, output không ổn định |
| 1.2 | Fix redundant logic producer | 🟠 NÊN LÀM | Code hygiene, tránh confusion |
| 1.3 | Fix news league_id hardcode | 🟠 NÊN LÀM | Data sai, query theo league miss |
| 1.4 | Fix Circuit Breaker HALF_OPEN | 🟡 NÊN LÀM | Edge case trong live pipeline |
| 2.1-2.3 | Đưa player_trend vào queue | 🟠 QUAN TRỌNG | Consistency kiến trúc + retry |
| 3.1-3.2 | Prompt versioning | 🟡 NÊN LÀM | Cần để chạy A/B test |
| 3.3-3.4 | Prompt Registry file | 🟡 NÊN LÀM | Single source of truth |
| 4.1 | Context awareness live_badge | 🟢 TỐT CÓ | Giảm duplicate content |
| 5.1 | text_utils clean output | 🟠 NÊN LÀM | Chất lượng output đầu ra |
| 5.2 | Monitoring log LLM | 🟢 TỐT CÓ | Observability production |
