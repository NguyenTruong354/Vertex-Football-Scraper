...
| `--scrape-only` | False | Chỉ scrape |
| `--load-only` | False | Chỉ load DB |
...
| `--quick-test` | False | limit 2 teams, 1 match |
...
Dùng `autorun.bat` để lên lịch qua Task Scheduler:

1. Win + R → `taskschd.msc`
2. **Create Task** → Tab **General**: đặt tên, tick *Run with highest privileges*
3. Tab **Triggers**: Daily → 06:00 (hoặc giờ tuỳ chọn)
4. Tab **Actions**: Start a program → `D:\Vertex_Football_Scraper2\autorun.bat`
5. Tab **Settings**: Stop if runs > 4 hours; restart every 30 min (x3)
...
cd /d "%PROJECT_DIR%"
"%PYTHON%" run_pipeline.py --league EPL >> logs\autorun_%date:~-4,4%%date:~-7,2%%date:~-10,2%.log 2>&1
```
...
# Hàng ngày 6:00 AM
0 6 * * * cd /path/to/project && .venv/bin/python run_pipeline.py --league EPL >> logs/autorun_$(date +\%Y\%m\%d).log 2>&1

# Hàng tuần Chủ nhật 3:00 AM — 5 giải
0 3 * * 0 cd /path/to/project && .venv/bin/python run_pipeline.py --league EPL LALIGA BUNDESLIGA SERIEA LIGUE1 >> logs/autorun_weekly.log 2>&1
```
...
`run_daemon.py` chạy liên tục không cần can thiệp. Tự nhận biết giờ thi đấu và điều chỉnh interval.
...
│    • Match hours (11:00–23:00 UTC): mỗi 30 phút        │
│    • Off hours:                     mỗi 2 giờ          │
│                                                         │
│  TIER 2 — FBref                                        │
│    • Mỗi 4 giờ (bất kể giờ nào)                       │
│                                                         │
