...
def load_match_lineups(conn, league_id: str = "EPL") -> int:
    path = _csv_path(league_id, "lineups")
    if not path.exists():
        logger.warning("  File không tồn tại: %s", path)
        return 0
    df = pd.read_csv(path, on_bad_lines="skip")
    count = _upsert(conn, "match_lineups", df, ["event_id", "player_id", "league_id"], league_id=league_id)
    logger.info("  match_lineups: %d rows upserted ← %s", count, path.name)
    return count


def load_match_passing_stats(conn, league_id: str = "EPL") -> int:
    path = _csv_path(league_id, "match_passing")
    if not path.exists():
        logger.warning("  File không tồn tại: %s", path)
        return 0
    df = pd.read_csv(path, on_bad_lines="skip")
    count = _upsert(conn, "match_passing_stats", df, ["event_id", "player_id", "league_id"], league_id=league_id)
    logger.info("  match_passing_stats: %d rows upserted ← %s", count, path.name)
    return count
